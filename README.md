...
- **快捷键重映射** - 临时修改快捷键映射
  ...
  **快捷键重映射限制**：
  ...
  为了保护系统稳定性和 Keymap 自身功能，以下快捷键**无法重映射**（快捷键面板中显示为灰色禁用按钮）：
  ...
  **Keymap 触发键** (3个)：
  - **⌘⌘** - 双击 Cmd 键打开快捷键面板
  - **⌥⌥** - 双击 Option 键打开快捷键面板（可在设置中选择）
  - **⌃⌃** - 双击 Control 键打开快捷键面板（可在设置中选择）

  > 💡 **为什么禁用这些快捷键？**
  > - 系统保留快捷键优先级最高，重映射无效且可能影响系统稳定性
  > - Keymap 触发键被重映射后会导致无法打开快捷键面板

  **验证规则**：除了上述快捷键外，重映射还会进行以下验证：
  - ❌ 源键和目标键不能相同
  - ❌ 不能创建循环映射 (A↔B)
  - ❌ 不能创建链式映射 (A→B→C)
  - ❌ 目标键也不能是系统保留快捷键

  鼠标悬停在快捷键面板的重映射按钮上，会显示提示信息："重映射此快捷键" 或 "此快捷键无法重映射"。
